const { BrowserWindow, Notification } = require('electron'); // Permite pintar una ventana de windows 
...
const createProduct = async (product) => {
    try {
      ...
      // Return the created Product para mostrar el resultado en pantalla
      return product;
    } catch (error) {
      console.log(error);
    }
...
function createWindow(){
    window = new BrowserWindow({
        ...
        height: 600,
        webPreferences: {
            nodeIntegration: true // para importar modulos de node
        }
    })
